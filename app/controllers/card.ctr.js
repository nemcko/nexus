 ...
 */
Controller.prototype.model = require('../models/card.mod.js');
...
Controller.prototype.assignData = function (fields, indata, user) {
    ...
    fields.cardtype = indata.cardtype;
    if (indata.accid) {
...
Controller.prototype.test = function (req, res) {
    Access.getView(req, function (err, acc) {
        if (err || acc.access.read === false) return callback(403, 'No read access.');
        var view = req.body
          , allOk = true
          , err;
        ...
        async.every(view.position, function (pos, callback) {
            var ok = true, ticket = {}
              , expr = 'ok=(' + Autom.constructJsQuery(pos, 'ticket') + ')';
            try {
                eval(expr);
            } catch (e) {
                ok = false;
                err = e;
                return callback(e, ok);
            }
            callback(null, ok);
        }, function (err, result) {
...
/*
 * List of views.
 * @method Controller.cardViews
 * @param req {Object} Request object
 * @param res {Object} Response object
 */
Controller.prototype.cardViews = function (req, res) {
    var access = {}
      , retitems = [];
