...
.component('nwFcolorButton', {
    ...
    template: '<select class=\"nw-select\" ng-model=\"$ctrl.fcolor\" ng-style = \"{\'color\': \'{{$ctrl.fcolor}} !important\' }\" ng-change=\"$ctrl.execCommand(\'ForeColor\', $ctrl.fcolor)\">\r\n<option ng-repeat=\"color in $ctrl.fcolors\" ng-disabled=\"$ctrl.editMode || $ctrl.disabled\"\r\nng-style = \"{\'color\':color.value}\" value=\"{{color.value}}\" \r\n>fg:{{color.name}}<\/option> \r\n<\/select>\t',
    controller: function () {
...
.config(['ngWigToolbarProvider', function (ngWigToolbarProvider) {
    ngWigToolbarProvider.addCustomButton('bcolor', 'nw-bcolor-button');
}])
.component('nwBcolorButton', {
bindings: {
    execCommand: '=',
editMode: '=',
disabled: '='
},
template: '<select class=\"nw-select\" ng-model=\"$ctrl.bcolor\" ng-style = \"{\'background-color\': \'{{$ctrl.bcolor}} !important\' }\" ng-change=\"$ctrl.execCommand(\'BackColor\', $ctrl.bcolor)\">\r\n<option ng-repeat=\"color in $ctrl.bcolors\" ng-disabled=\"$ctrl.editMode || $ctrl.disabled\"\r\nng-style = \"{\'background\':color.value}\" value=\"{{color.value}}\" \r\n>bg:{{color.name}}<\/option> \r\n<\/select>\t',
controller: function () {
